# app/services/german_authority_engine/integration/authority_endpoints.py - COMPLETE FIXED VERSION
...
# FIXED: Proper imports at the top of the file
...
class Big4AuthorityEndpoints:
    ...
    def __init__(self):
        """Initialize Big 4 Authority Endpoints with proper error handling"""
        logger.info("Initializing Big 4 Authority Endpoints")
        
        # IMPROVED: Better import handling to avoid circular imports
        self.detector = None
        self.analyzer = None
    ...
    def _initialize_components(self):
        ...
        try:
            # Import detector and analyzer with error handling
            ...
            # Components remain None, which is handled in methods
        except Exception as e:
            logger.error(f"Unexpected error initializing Big 4 components: {e}")
    
    def _check_availability(self):
        """Check if Big 4 components are available"""
        if not self.detector or not self.analyzer:
            raise HTTPException(
                status_code=503,
                detail="Big 4 Authority Engine not available. Please check system configuration."
            )
        ...
        try:
            ...
            return {
                ...
                "compliance_analysis": {
                    ...
                    "requirements_met": analysis.requirements_met,
                    "industry_specific_guidance": analysis.industry_specific_guidance,
                    "enforcement_patterns": getattr(analysis, 'enforcement_patterns', []),
                    "audit_preparation_steps": getattr(analysis, 'audit_preparation_steps', []),
                    "estimated_penalty_range": analysis.estimated_penalty_range,
                    "contact_information": getattr(analysis, 'contact_information', {})
                },
                "recommendations": {
        ...
        try:
            ...
            return {
                ...
                "alternative_authorities": [
                    ...
                    for auth, score in getattr(detection_result, 'alternative_authorities', {}).items()
                ],
                "business_profile": {
                    "industry": industry,
                    "location": company_location,
    ...
    async def get_all_big4_authorities_info(self) -> Dict[str, Any]:
        """
        Big 4 German Authorities Information - FIXED: Complete implementation
    ...
    async def _process_uploaded_files(self, files: List[UploadFile]) -> List[Any]:
        ...
        for file in files:
            ...
            # FIXED: Check total size limit using the correct property from settings
